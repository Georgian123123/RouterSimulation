...
static bool compare(const route_table rt1,const route_table rt2) {
	if (rt1.prefix < rt2.prefix) {
		return true;
	} else if (rt1.prefix == rt2.prefix) {
		return (rt1.mask > rt2.mask);
	}
	return false;
}
...
int read_rtable(struct route_table *rTable) {
	...
	char lines[100];
	int i;
	for (i = 0; fgets(lines, sizeof(lines), f); ++i) {
		char prefix[20], next_hop[20], mask[20];
		int interf;
		sscanf(lines, "%s %s %s %d", prefix, next_hop, mask, &interf);
	...
	int rTable_len = i;
	fclose(f);
	std :: sort(rTable , rTable + rTable_len, compare);
	return rTable_len;
}
